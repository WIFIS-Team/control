...
# -- Read temperatures from temperature controller at interval set by 'interval' variable
# -- Generate setpoints that increases at a defined by the 'warming_gradient' variable, and update this setpoint based on the real detector temperatures at interval defined by the 'setpoint_update_interval' variable
# -- Setpoint shall not be set to more than 'dt_above_worksurface' degrees above the worksurface
# -- Setpoint shall not be set to higher than the temperature set in 'setpoint_ceiling' variable. 
# -- Turn off heaters if temperature gradient is warming up faster than specified in max_gradient
# -- Turn off heater if detector temperature is higher than that set in the temperature_limit variable
...
#            stored_Temperature = (float(last_read[2])+float(last_read[3]))/2
            stored_Temperature = float(last_read[3])
        ...
        average_dT_dt = dT_dt_B
        g_dT_dt = average_dT_dt
        
        print 'DT/dt: ',  average_dT_dt
        
        Td = float(current_read[3]) #detector temperature is the average of input A and B
        Twork = float(current_read[5]) #worksurface temperature is input D
...
#                print 'Trigger 1' #====Testing
        ...
        new_setpoint = stored_Temperature + warming_gradient*((time_since_stored_Temperature_recorded+actual_setpoint_update_interval)/60.0)
        
        if new_setpoint >= Twork+dt_above_worksurface:
            new_setpoint = Twork+dt_above_worksurface
            print 'Keeping setpoint to less than 3K above worksurface'
        
        if new_setpoint >= setpoint_ceiling:
            new_setpoint = setpoint_ceiling
            print 'Keeping setpoint to the ceiling'
...
def write_html(sensor_readings):
    ...
    html_file.write('<h2> Warming script running </h2>'+'\n')
...
#        remotepath = '/disk3/www/kema/t_stream.html'
#        remotepath_img = '/disk3/www/kema/history_plot.jpg'
#        remotepath_img_2 = '/disk3/www/kema/history_plot_heater.jpg'
#        remotepath_img_3 = '/disk3/www/kema/history_plot_Tgrad.jpg'
#        remotepath_img_4 = '/disk3/www/kema/pressure_plot.jpg'
        remotepath = '/Users/suresh/Sites/wifis/t_stream.html'
        remotepath_img = '/Users/suresh/Sites/wifis/history_plot.jpg'
        remotepath_img_2 = '/Users/suresh/Sites/wifis/history_plot_heater.jpg'
        remotepath_img_3 = '/Users/suresh/Sites/wifis/history_plot_Tgrad.jpg'
        remotepath_img_4 = '/Users/suresh/Sites/wifis/pressure_plot.jpg'
...
def generate_history_plot(new_data):
    ...
    if queue_length < 2:
        ...
        plt.grid(True)
        p1,  = plt.plot((numpy.array(timestamp_list,copy=True)-time_now)/60,  numpy.array(input_B_list,copy=True),  'bs')
        p2,  = plt.plot((numpy.array(timestamp_list,copy=True)-time_now)/60,  numpy.array(input_D_list,copy=True),  'g^')
        p3,  = plt.plot((numpy.array(timestamp_list,copy=True)-time_now)/60,  numpy.array(input_E_list,copy=True),  'ro')
        plt.legend(['Detector', 'Optics', 'Worksurface'], loc=9)
        ...
        p2,  = plt.plot((numpy.array(timestamp_list,copy=True)-time_now)/60,  numpy.array(dT_dt_list,copy=True)*0.0+warming_gradient,  'b--')
        ...
        plt.legend(['Detector', 'ASIC', 'Getter'], loc=9)
...
def record_readings_to_file(sensor_readings):
    ...
    returnedString = sensor_readings
    currentTime = time.time()
#    if currentTime > startTime+3600: #start a new file every hour
#        startTime = currentTime
#        f.close()
#        f=open(time.asctime().replace(':', '-')+'.csv', 'w')
#        outData = csv.writer(f, dialect='excel')
#        outData.writerow(outFileHeader)
#        print 'New Recording File Created'
