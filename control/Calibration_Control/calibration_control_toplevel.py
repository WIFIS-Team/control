...
#NOTE: the statuses of the componants displayed by this app won't be updated if their power is disrupted through other sources, keep this in mine (could add this function by adding an update_status() function like in power_control.py but this would probably cause the whole code to lag annoyingly. 
...
class MainApplication(Frame): #this class holds all of the gui into and button functions. not really sure why it's a class. Jason did it this way and I copied
    ...
    def create_widgets(self): #create all the buttons and labesl and stuff
        #create dividing lines to keep gui clean (sticky makes them extend all the way)
        ttk.Separator(self,orient=VERTICAL).grid(rowspan=12, column=1, sticky="ns")

        ttk.Separator(self,orient=HORIZONTAL).grid(row=2, column=2, columnspan=3, sticky="ew")
        ttk.Separator(self,orient=HORIZONTAL).grid(row=5, column=2, columnspan=3, sticky="ew")

        #Top level control (buttons that do multiple things each leads to function bellow)
        ######################################################

        #Label(self, text="Enter Calibration Mode", font='bold').grid(row=0, column=0, padx=15)

        self.c1=Button(self, text="Enter Calibration Mode", command=self.f1,relief=RAISED)
        self.c1.grid(row=0, column=0, padx=15)  
    
        #Label(self, text="Prepare to take Flats", font='bold').grid(row=2, column=0, padx=15)

        self.c2=Button(self, text="Prepare to take Flats", command=self.f2, relief=SUNKEN)
        self.c2.grid(row=3, column=0, padx=15)

        #Label(self, text="Finished taking Flats", font='bold').grid(row=4, column=0, padx=15)

        self.c3=Button(self, text="Finished taking Flats", command=self.f3, relief=SUNKEN)
        self.c3.grid(row=5, column=0, padx=15)

        #Label(self, text="Prepare to take Arcs", font='bold').grid(row=6, column=0, padx=15)

        self.c4=Button(self, text="Prepare to take Arcs", command=self.f4,relief=SUNKEN)
        self.c4.grid(row=7, column=0, padx=15)

        #Label(self, text="Finished taking Arcs", font='bold').grid(row=8, column=0, padx=15)

        self.c5=Button(self, text="Finished taking Arcs", command=self.f5, relief=SUNKEN)
        self.c5.grid(row=9, column=0, padx=15)

        #Label(self, text="Exit Calibration Mode", font='bold').grid(row=10, column=0, padx=15)

        self.c6=Button(self, text="Exit Calibration Mode", command=self.f6, relief=SUNKEN)
        self.c6.grid(row=11, column=0, padx=15)


        #Sphere stuff (label if spehre's on or off allows user to turn it on and off by itself)
        ######################################################
        Label(self, text="Integrating Sphere", font='bold').grid(row=0, column=2, padx=15)

        Button(self, text="On/Off", command=self.toggle_sphere, width=5).grid(row=1, column=2)
        Label(self, text="Status:").grid(row=1, column=3)
        
        self.status_sphere = Label(self, text='OFF', fg ='red' ) #this will update if sphere turned on by top level controls or by the button in this section NOT if changed elsewhere
        self.status_sphere.grid(row=1, column=4)

        #Arc Lamp stuff (labels if arc lamp on or off lets user turn it on and off individually)
        ######################################################
        Label(self, text="Arc Lamp", font='bold').grid(row=3, column=2, padx=15)

        Button(self, text="On/Off", command=self.toggle_arc, width=5).grid(row=4, column=2)
        Label(self, text="Status:").grid(row=4, column=3)
        
        self.status_arc = Label(self, text='OFF', fg ='red' ) #this will update if arc lamp turned on by top level controls or by the button in this section
        self.status_arc.grid(row=4, column=4)       

        #Flipper stuff (this is pretty much identical to flipper_gui.py, see that code for comments...) 
        ######################################################
        ...
        Button(self, text="On/Off", command=self.fliponoff,width=5).grid(row=7, column=2)
        
        Label(self, text="Flipper 1 (In Calibration Box)", font='bold',anchor=W).grid(row=8, column=2,columnspan=2, padx=15,sticky="ew")
        
        self.b1=Button(self, text="Pos1 (Integrating Sphere)", command=self.flip1pos1,relief=SUNKEN)
        self.b1.grid(row=9, column=2)

        self.b2=Button(self, text="Pos2 (Arc Lamp)", command=self.flip1pos2, relief=RAISED)
        ...
        Label(self, text="Flipper 2 (On WIFIS)", font='bold',anchor=W).grid(row=10, column=2,columnspan=2, padx=15,sticky="ew")

        self.b3=Button(self, text="Pos1 (Observation Mode)", command=self.flip2pos1, relief=SUNKEN)
        self.b3.grid(row=11, column=2)

        self.b4=Button(self, text="Pos2 (Calibration Mode)", command=self.flip2pos2, relief=RAISED)
...
#Top level control (each button does a series of steps to do calibratoin and clean up afterward)
######################################################
...
#Flipper stuff (see flipper_gui.py for explenations of theses) tells flippers to move to a given positoin and shows when it's moving and when its stopped 
######################################################
    def flip1pos2(self):
        ser.write(bytes('N'))
